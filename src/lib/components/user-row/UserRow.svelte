<script lang="ts">
	...
	const deleteUser = async () => {
		const result = await connectionWrapper(async () => {
			const r: boolean = await invoke('delete_user', { id: Id });
			return r;
		})();
		if (!result) {
			toast.error(`An arror occured while trying to remove user with Id ${Id}`);
			return;
		}

		toast.success(`Successfully removed user with Id ${Id}`);
		deletef();
	};
	...
	const editUser = async () => {
		...
		const result = await connectionWrapper(async () => {
			const r: boolean = await invoke('edit_user', {
				user: {
					id: Id,
					tier_id: newTierId,
		...
		if (!result) {
			toast.error(`An error occured while trying to edit user with Id ${Id}`);
			return;
		}
		toast.success(`Successfully edited user with Id ${Id}`);
		editMode = false;
		Nickname = newNickname;
		Email = newEmail;
		Password = newPassword;
		...
		edit({ Tier: TierId, Nickname, Email, Password, Id });
	};
			...
			<Table.Cell>
				<Tooltip.Root>
					<Tooltip.Trigger class="w-full">
						<span class="select-text">{TierId}</span>
					</Tooltip.Trigger>
					<Tooltip.Content>
